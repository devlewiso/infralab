...
This is not a hobby project—it's a **production-grade infrastructure lab** that demonstrates real-world systems engineering, operational excellence, and architectural depth. Every component is chosen deliberately, every boot sequence is orchestrated, and every failure scenario is planned for.
...
Internet (Starlink Mini)
        ...
        ▼ 1000FDX
┌───────────────────┐
│  ER605 Router     │ MAC: 1C-61-B4-2C-1B-E3
│  WAN1             │
└────────┬──────────┘
         │ Port #5 → Port #1
         ▼ 1000FDX
┌───────────────────┐
│  Cerebro          │ TP-Link SG3428
│  (Main Switch)    │ 28-Port Managed
└┬──────┬───────┬───┘
 │      │       │
 │      │       └──────► Port #4: Omada Controller
 │      │
 │      └──► Port #24: Antenitas (EAP225-Outdoor)
 │                      ├── WiFi: 6(b/g/n), 36(a/n/ac)
 │                      ├── Client Group 6 (WiFi)
 │                      └── Camera Group 4 (WiFi)
 │
 └──────► Client Group 7 (Wired)
```
...
## 🏗️ Core Architecture: Layered Boot Sequence
...
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐           │
│  │ CT 101      │ │ CT 102      │ │ CT 120      │           │
│  │ Omada21     │ │ Twingate   │ │ Monitoring30│           │
│  │ Network     │ │ Zero Trust │ │ Observability│          │
│  └─────────────┘ └─────────────┘ └─────────────┘           │
└──────────────────────────┬──────────────────────────────────┘
                           │ depends on
                           ▼
...
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐           │
│  │ CT 159      │ │ CT 155      │ │ CT 124      │           │
│  │ PostgreSQL  │ │ NodeBB     │ │ DevOps Lab  │           │
│  │ Database    │ │ Forum      │ │ CI/CD       │           │
│  └─────────────┘ └─────────────┘ └─────────────┘           │
└──────────────────────────┬──────────────────────────────────┘
                           │ depends on
                           ▼
...
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐           │
│  │ CT 122      │ │ CT 243      │ │ CT 289      │           │
│  │ Web-Lab22   │ │ Ollama     │ │ Ridemotohub│            │
│  │ Web Services│ │ AI/LLM     │ │ App Platform│           │
│  └─────────────┘ └─────────────┘ └─────────────┘           │
└──────────────────────────┬──────────────────────────────────┘
                           │ depends on
                           ▼
...
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐           │
│  │ CT 100      │ │ CT 200      │ │ CT 104      │           │
│  │ CasaOS      │ │ Home       │ │ NVR69       │           │
│  │ Media Server│ │ Assistant  │ │ Surveillance│           │
│  └─────────────┘ └─────────────┘ └─────────────┘           │
└─────────────────────────────────────────────────────────────┘
```
...
|-------|------|-------------|--------|
| 101 | Omada21 Network Controller | TP-Link Omada network management | 🟢 Active |
| 102 | twingate-connector | Zero Trust Access gateway | 🟢 Active |
| 120 | Monitoring30 | Infrastructure observability (Prometheus/Grafana) | 🟢 Active |
...
| CT ID | Name | Description | Status |
|-------|------|-------------|--------|
| 159 | dbEmpleosdelatlantico | PostgreSQL Database | 🟢 Active |
| 155 | nodebb Forum Backend | Community platform backend | 🟢 Active |
| 124 | devops-lab | Development and CI/CD environment | 🟢 Active |
...
| CT ID | Name | Description | Status |
|-------|------|-------------|--------|
| 122 | web-lab22 | Web Services & Application Frontend | 🟢 Active |
| 243 | ollamaServer | Local AI/LLM Inference (Privacy-first) | 🟢 Active |
| 289 | ridemotohub | Custom Application Platform | 🟢 Active |
...
| CT ID | Name | Description | Status |
|-------|------|-------------|--------|
| 100 | CasaOSMedia105 | Media Server | 🟢 Active |
| 200 | homeassistant119 | Home Automation | 🟢 Active |
| 104 | NVR69 | Video Surveillance System | 🟢 Active |
...
│   Connector     │     (No VPN, Identity-based)
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│   VLAN          │ ◄── Network Segmentation
│   Segmentation  │     (Isolated traffic)
└────────┬────────┘
         │
         ▼
┌─────────────────┐
...
## ⚙️ Architecture Insights & Constraints
...
Layer 1 → Wait for health → Layer 2 → Wait → Layer 3 → Wait → Layer 4
```
